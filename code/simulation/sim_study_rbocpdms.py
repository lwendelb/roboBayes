...
if normalize:
    data = (data - np.mean(data))/np.sqrt(0.001) #/np.sqrt(np.var(data))
...
alpha_rld = 0.5
alpha_param = 0.5
rld = "power_divergence"#"kullback_leibler"#"power_divergence"

prior_mean_scale, prior_var_scale = np.mean(data), 10 #np.sqrt(np.var(data))
cp_intensity = 270
...
if mode == "DPD" or mode == "both":
    model_universe = model_universe + [BVARNIGDPD(
                 ...
                 general_nbh_sequence=np.array([[[]]]*S1*S2),
                 general_nbh_restriction_sequence = np.array([[0]]),
